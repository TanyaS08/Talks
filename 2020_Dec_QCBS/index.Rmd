---
title: "Exploring the complexity of ecological networks using SVD entropy"
subtitle: | 
author: |
  <p style="font-size:23pt;"><u>Tanya Strydom</u><sup>1,2</sup><br> 
  Gullio V. Dalla Riva<sup>3</sup> & 
  Timothée Poisot<sup>1,2</sup></p><br> 
  <body style="font-size:8pt;"><small><sup>1</sup>Département de Sciences Biologiques, Université de Montréal; <sup>2</sup>Québec Centre for Biodiversity Sciences; <sup>3</sup>School of Mathematics and Statistics, University of Canterbury</small></body>
date: |
  <a href="https://twitter.com/tanyas_08"><i class="fa fa-twitter"></i></a><a href="https://twitter.com/tanyas_08"> @TanyaS_08 </a>
output:
  xaringan::moon_reader:
    lib_dir: libs
    css: xaringan-themer.css
    nature:
      highlightStyle: github
      highlightLines: true
      countIncrementalSlides: false
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE, eval = FALSE, collapse = TRUE, comment = ">")
options(htmltools.dir.version = FALSE)
htmltools::tagList(rmarkdown::html_dependency_font_awesome())
```

```{r xaringan-themer, include=FALSE, warning=FALSE}
library(xaringanthemer)
style_duo_accent(
  primary_color = "#4B384C",
  secondary_color = "#FF961C"
)
```

# The complexity of communities

.pull-left[
Network theory provides a powerful approach to understanding the complexity of ecological networks
]


<span 
  style="
  margin-left:-10%; 
  position: absolute; 
  top: 17%">
  <img src="img/community.png" width="80%" />
</span>

???


--

<span style="margin: 0;
  position: absolute;
  bottom: 10%;
  text-align: left;
  left: 6%;"> We can use network structure <br>
  to infer ecological properties <br>
  and processes
</span>
  
<span 
  style="
  margin-left:-10%; 
  position: absolute; 
  top: 17%">
  <img src="img/Network.png" width="80%" />
</span>

<span style="margin: 0;
  position: absolute;
  top: 20%;
  text-align: center;
  right: 5%;"> Complexity = stability
</span>
  
???
This notion doesn't really hold though...

---

# A traditional definition of complexity

Assume that a system is complex because it has many parts *i.e.* measuring structural complexity

<span 
  style="
  margin-left:0%; 
  position: absolute; 
  top: 35%">
  <img src="img/structure.png" width="95%" />
</span>

---

# Taking a physical approach to complexity

Instead we can measure the 'physical complexity' *i.e.* the amount of information required to encode the system

<span 
  style="
  margin-left:0%; 
  position: absolute; 
  top: 42%">
  <img src="img/Haring.jpg" height="350 px" />
</span>

<span 
  style="
  margin-left:50%; 
  position: absolute; 
  top: 42%">
  <img src="img/Basquait.png" height="350 px" />
</span>
---

# Estimating complexity using SVD

$$\mathbf{A} = \mathbf{U}\cdot\mathbf{\Sigma}\cdot\mathbf{V}^T$$

<span 
  style="
  margin-left:00%; 
  position: absolute; 
  top: 42%">
  <img src="img/SVD.png" width="95%" />
</span>

---

# SVD Entropy as internal complexity

Using the $\sigma$ values from $\mathbf{\Sigma}$ and following Pielou's evenness  (Pielou 1975) we can ensure that $\sigma$ values are less than unity:

$$
s_i = \sigma_i/\text{sum}(\sigma)
$$

--

and estimate entropy following Shannon (1948) as:


$$
J = -\frac{1}{\ln(k)}\sum_{i=1}^k s_i\cdot\ln(s_i)
$$

--

High values of SVD entropy reflects that all vectors are equally important, *i.e.* we cannot efficiently compress the network, indicating high complexity 

???

where k is the max rank

---

# SVD Entropy as internal complexity

<span 
  style="
  margin-left:0%; 
  position: absolute; 
  top: 30%">
  <img src="img/SVD_vis.png" width="90%" />
</span>

---
# SVD Entropy of Networks

Ecological networks are extremely complex

<span 
  style="
  margin-left:0%; 
  position: absolute; 
  top: 20%">
  <img src="img/interactiontype_v_entropy.png" width="93%" />
</span>

---
# Network Structure Captures Network Complexity 


<span 
  style="
  margin-left:15%; 
  position: absolute; 
  top: 25%">
  <img src="img/others_v_entropy.png" width="95%" />
</span>

--

 .footnote[But then things begin to tell a different story...]
---
# Complex does not mean robust

--

<span 
  style="
  margin-left:10%; 
  position: absolute; 
  top: 30%">
  <img src="img/entropy_v_AUCall.png" width="75%" />
</span>

???

[structural measure of complexity are related to robustness (simulating extinctions)... BUT we don't see this with SVD Entropy]


---

# What next?

+ Complexity is a slippery term to define

+ Different measures of complexity capture different ecological processes

  * *e.g.* simulating robustness to extinctions
  
+ SVD Entropy may provide a starting point to unifying the definition of complexity

.footnote[
For a more complete breakdown the preprint can be found at [doi.org/fmpt](https://doi.org/fmpt)
]
